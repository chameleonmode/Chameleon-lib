...
async function OnLoad() {
    await chrome.storage.sync.set(extSettings);
    ...
    await debuggerStatus(tab.id, async function(debugStatus)
    {
      ...
      if((tab && tab.url && tab.url.indexOf("chrome://") < 0))
      {
          chrome.storage.local.get({
              timezone: extSettings.timezoneId,
          random: false,
          myIP: false,
          locale: 'en-US'
        }, async prefs => {
          ...
          if (extSettings.geoSpoofing) {
              chrome.storage.local.get({
                  latitude: extSettings.latitude,
                  longitude: extSettings.longitude,
                  accuracy: extSettings.accuracy
              }, async prefs => {
...
async function debuggerStatus(tabId,callback)
{
  await chrome.debugger.getTargets((targets) => {
    const targetTabId = tabId;  // Replace with the ID of the target tab
    const isDebuggerAttached = targets.some(target => 
      target.tabId === targetTabId && target.attached
    );
    callback(isDebuggerAttached);
  });
}
